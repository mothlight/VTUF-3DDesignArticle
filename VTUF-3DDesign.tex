...
[Explain what ]. Intercomparison Phase 4 error analysis provides RMSE values for fluxes $Q^{*}$, $Q_{H}$, $Q_{G}$, and $Q_{E}$ for three classifications of urban models. These three categories are of models that do not consider vegetation, those that model vegetation using tiles[May need to elaborate a bit.], and models that integrate vegetation [Again, elaborate for reader.]into the urban area (referred to as classifications IntercomparisonNoVeg, IntercomparisonTiled, and IntercomparisonIntegrated, described in Table \ref{tab:simscompared}).
